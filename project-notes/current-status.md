...
- The Results/Picks direction has been revised away from the expanded first-pick treatment. `SearchResultRows.jsx` now renders every focused pick with the same ranked row/card structure, fixed image/rank lane, constrained title/reason lines, and consistent `View details` affordance. `ResultsScreen.jsx` owns a sticky selected-result panel above the scrolling rows; selection follows the highest visible result row while scrolling, and the selected row is marked with a left accent line plus lighter cream shading and accessibility label. The selected panel is currently a side-by-side hero card with a larger, lightly framed product image, title plus rationale as the emotional focus, and price/rating moved lower as quieter metadata. The smaller result rows are intentionally lighter scan cards with reduced height, quieter rationale copy, compressed metadata, and softened image framing. Long selected titles currently get a larger selected-panel allowance and are clamped at a word boundary before rendering so they end with a cleaner `...` rather than cutting a word in half where practical. This keeps the shortlist scannable and avoids making pick #1 feel over-certified. The latest selected-panel/row visual hierarchy pass passed focused Results tests and Android Expo export.
