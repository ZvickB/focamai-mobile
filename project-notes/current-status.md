...
- The Results/Picks direction has been revised away from the expanded first-pick treatment. `SearchResultRows.jsx` now renders every focused pick with the same ranked row/card structure, fixed image/rank lane, constrained title/reason lines, and consistent `View details` affordance. `ResultsScreen.jsx` owns a sticky selected-result panel above the scrolling rows; selection follows the highest visible result row while scrolling, and the selected row is marked with a left accent line plus lighter cream shading and accessibility label. The selected panel is currently a side-by-side experiment with title/price/rating on the left and a larger product image on the right, without increasing the panel height or showing a separate `Selected result` eyebrow. Long selected titles currently get a larger selected-panel allowance and are clamped at a word boundary before rendering so they end with a cleaner `...` rather than cutting a word in half where practical. This keeps the shortlist scannable and avoids making pick #1 feel over-certified. The latest selected-panel layout passed focused Results tests and Android Expo export.
...
- A reusable `GuidanceText` / `fontGuidance` token exists for future guidance-accent typography, but it currently points back to Manrope and does not load a second font. Current usage is limited to the short refine support line and the `Why these picks?` rationale body; keep any future accent out of buttons, navigation, inputs, product titles, prices, specs, chips, badges, and dense card content.
...
- Refine notes now explicitly allow up to 500 characters on mobile, matching the shared web/backend detail limit. AI refinement chip prompts are clamped to that limit before filling the note box. Hard-constraint discovery refresh still has to call the discovery endpoint with a product-query-shaped string, so mobile now caps that internal refresh query to the existing 80-character discovery query limit while still sending the full refine notes to finalize.
...
The individual detail screen has been redesigned using the May 13 mockup only as taste reference and the May 19 Search/Refine/Results direction as the active constraint. `SearchResultDetailScreen` now hides the native stack header and owns a native detail header with back-to-picks navigation and centered local PNG wordmark. The page leads with the product image, pick/source context, title, and compact price/rating/review facts; then keeps `Why this pick`, `Worth knowing`, feature notes, retailer CTA/disclosure, and at-a-glance metadata in a calmer decision-support order. It uses Lucide icons for light structure, but does not add saved, compare, share, profile, confidence percentages, tabs, bottom nav, or Amazon-specific CTA language. Candidate-id lookup, stale route-snapshot fallback, enrichment-aware fallback copy, retailer CTA/disclosure, fixed retailer footer, backend/controller contracts, navigation flow, query-quality polling, retry advice, hard-constraint refresh, and the 6-result cap are unchanged.
