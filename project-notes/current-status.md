...
- Refine notes now explicitly allow up to 500 characters on mobile, matching the shared web/backend detail limit. AI refinement chip prompts are clamped to that limit before filling the note box. Hard-constraint discovery refresh still has to call the discovery endpoint with a product-query-shaped string, so mobile now caps that internal refresh query to the existing 80-character discovery query limit while still sending the full refine notes to finalize. A broader UX debt remains: backend validation/API errors during the guided flow should become inline recovery states instead of a scary full error-screen experience.
