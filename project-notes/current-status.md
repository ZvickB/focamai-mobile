...
A temporary dev-only background palette tester is implemented for visual comparison across Search, Refine, Results/Picks, and Detail. `src/theme/BackgroundPaletteContext.jsx` defines eight named warm-neutral options using semantic tokens (`appBackground`, `cardBackground`, `borderSubtle`) and exposes an in-memory cycle control rendered only in `__DEV__`. `App.js` wraps the app in `BackgroundPaletteProvider`; `RootNavigator.jsx` feeds the selected `appBackground` into the React Navigation theme/header/content colors; `ScreenContainer` uses the same `appBackground` for the shared screen shell while leaving typography, layout, spacing, cards, buttons, and accent colors unchanged. This is not a user setting and is not persisted.

The dev palette tester passed `npm test -- --runInBand src/search/__tests__/SearchEntrySection.test.jsx src/search/__tests__/SearchResultsSection.test.jsx src/screens/__tests__/SearchResultDetailScreen.test.jsx`, `node --check App.js`, `node --check index.js`, `node --check tailwind.config.js`, and Android Expo export; `.expo-export-check` was removed afterward.

The temporary background experiment is currently narrowed to five options after removing Warm Stone, Subtle Oat, and Pale Sand. Soft Porcelain is the default active test option. Search now has a Search-only top warmth wash using `react-native-svg`, and `ScreenContainer` accepts an optional `backgroundElement` so this stays out of the other screens. Search and Refine share `SearchFlowProgressCue`; on Search it sits near the top above the wordmark/hero, matching the existing `Search / Refine / Picks` step cue instead of using a separate text-only cue. The experiment passed `npm test -- --runInBand src/search/__tests__/SearchEntrySection.test.jsx src/search/__tests__/SearchRefineSection.test.jsx`, `node --check App.js`, `node --check index.js`, and Android Expo export; `.expo-export-check` was removed afterward.
