...
The May 19 Results payoff pass is implemented. `ResultsScreen` now hides the native stack header and uses the same native header pattern as Refine: back-to-refine navigation, centered local PNG wordmark, quiet icon-only Settings access, and a thinner `Search / Refine / Picks` progress cue with only a small ember active accent. The Results intro now leads with `Your focused picks`, a short assistant-style line, and a quiet query/notes/count orientation strip instead of a badge-like context panel. `SearchResultsSection` no longer wraps product cards inside an outer card; it renders `Six worth considering` followed by standalone product cards, keeps early preview behind the existing reveal, and calmly explains when fewer than six credible options come back. `SearchResultRows` keeps rank, product image/fallback, title, provider, price, rating/review fallbacks, a short fit/caveat/feature preview, detail affordance, retailer CTA, and affiliate disclosure while removing the louder first-pick treatment, confidence-style language, metadata chip density, and nested reason card. Backend contracts, controller flow, explicit store preference behavior, query-quality polling, retry advice, enrichment hydration, candidate-id lookup, retailer CTA/disclosure, and the 6-result cap are unchanged.

The May 19 Results page visual pass passed `npm test -- --runInBand src/search/__tests__/SearchResultsSection.test.jsx`, direct `node --check` on `App.js` and `index.js`, and `npx expo export --platform android --output-dir .expo-export-check`; `.expo-export-check` was removed afterward. Direct `node --check` on touched `.jsx` files is not supported by this Node setup, so Android export remains the JSX parse/bundle check.
