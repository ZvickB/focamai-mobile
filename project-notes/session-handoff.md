...
**Privacy / Play submission:** The 2026-07-02 cross-repo work updated both the mobile Privacy screen and canonical web `/privacy` page and implemented secure self-service account deletion through mobile Settings → Account, authenticated Render `DELETE /api/account`, and public `/delete-account`. The documented account-owned tables cascade from `auth.users`; anonymous operational records without a user ID remain. Local tests/builds pass, but live deployed Supabase/Render and physical-device verification is still required before Google Play publication.
...
**Retry advice query length is intentionally 80 characters.** The backend clamps suggestions to the shared product-query limit and mobile normalizes them before automatically starting refreshed discovery. Do not raise retry advice alone unless the shared limit changes everywhere.
...
- Mobile now has email/password Supabase auth and account-backed history parity for saved searches. Signed-out users keep device-local `AsyncStorage` history; signed-in users use Supabase `saved_searches`; local entries migrate into the account on sign-in. Mobile stores Supabase session data through `expo-secure-store`, not plaintext `AsyncStorage`, and migration removes only the entries that were actually migrated so sign-out/cancellation mid-sync does not wipe new local history. Google sign-in is still stubbed on mobile until an Expo auth-session slice is deliberately added.
...
- Production crash reporting is wired through Sentry but cannot ship accidentally unconfigured: EAS requires `EXPO_PUBLIC_SENTRY_DSN`, `SENTRY_ORG`, `SENTRY_PROJECT`, and `SENTRY_AUTH_TOKEN`. Reporting is crashes/serious errors only, with default PII, breadcrumbs, tracing, profiling, app hangs, automatic sessions, replay, and feedback disabled. Sentry `8.17.2` is intentionally excluded from Expo's version check because the Expo-recommended `7.2.0` serializer fails with the installed Worklets/Reanimated combination. Android export passes; live project setup, EAS values, a signed-build test event, and the Sentry retention setting are pending. Once configured for the release, Google Play Data Safety should include **Crash logs** as collected, required, retained, used for Analytics (crash diagnosis), encrypted in transit, and not shared when Sentry is acting only as Focamai's service provider.
...
- The Results retry correction surface is an expandable `Improve these picks` prompt. Keep one correction note and one `Update my picks` action; do not restore the generated-query confirmation or correction chips without revisiting the product decision.
...
- Shopping Region is intentionally a quick selector, not a long settings/catalog surface. Active mobile choices are United States (`amazon.com`), Canada (`amazon.ca`), and India (`amazon.in`). India is enabled without an Associates tag for tester access, so its Amazon links remain usable but must not show the Associates earnings disclosure; US and Canada remain tagged. Inactive saved marketplaces such as `amazon.co.uk` fall back to `amazon.com`. Keep explicit selection with no location permission, the compact divider-list treatment, and the existing return-to-Search behavior.
