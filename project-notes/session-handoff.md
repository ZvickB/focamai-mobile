...
- Refine and Results retry keyboard overlap are addressed in code. `FollowUpScreen` wraps the complete form-plus-footer layout in a platform-aware `KeyboardAvoidingView`, keeping `Update my picks` above the keyboard; `ResultsScreen` does the same for retry correction/suggestion inputs and actions. Android explicitly uses `softwareKeyboardLayoutMode: "resize"`. Recheck both surfaces on a physical Android device because keyboard height and navigation mode vary by device.
...
- Results retry advice now renders the AI-generated suggested query as an editable input as soon as advice appears, with no separate `Edit first` step. `Search this suggestion` starts the fresh guided search from the edited field value, and the field remains capped at the shared 80-character product-query limit. Focused Retry/Results tests plus Android Expo export passed.
...
- The Results retry correction surface is an expandable friendly prompt, not a labeled recovery card. Keep `Want to correct the direction?` and the `Say what felt off...` helper visible; keep the correction note box and `Get AI search suggestion` button hidden until expanded. Do not reintroduce correction chips unless they visibly improve the generated query path. Retry advice renders underneath as the AI-generated `Suggested next search`, with the suggested query already editable and a `Search this suggestion` action.
...
- Shopping Region is now intentionally a quick selector, not a long settings/catalog surface. For Amazon Associates compliance, keep active mobile store choices limited to affiliate-tagged Amazon marketplaces for now: United States (`amazon.com`) and Canada (`amazon.ca`). Stale saved preferences for untagged marketplaces such as `amazon.co.uk` fall back to `amazon.com`. Keep the compact divider-list treatment and clear selected check state while unselected rows stay secondary and easy to scan. Return-to-Search navigation is unchanged.
...
- A bounded mobile-native retry-advice path is now implemented:
  ...
  - `src/search/SearchRetrySection.jsx` keeps the retry correction note as the entry point, asks AI for a suggested query, shows suggested query/rationale when available, renders the suggested query as editable immediately, supports `Search this suggestion`, and shows lightweight `Keeping:` reassurance tags
