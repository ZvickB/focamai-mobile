...
- Refine-note length handling is now hardened for AI chips. Mobile allows/clamps follow-up notes at 500 characters, matching the shared web/backend detail limit. `buildConstraintRefreshQuery()` caps only the internal hard-constraint refresh query to the discovery endpoint's 80-character product-query limit so a long AI chip prompt does not trigger the ugly error path before finalize. Keep the full notes available for finalize.
