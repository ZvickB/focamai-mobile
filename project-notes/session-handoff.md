...
- Follow-up, Results, and Detail stack headers now use shared `AppHeader`, `HeaderBackButton`, and `IconButton` primitives. The header has fixed left/right slots and an absolutely centered wordmark, and the Results header is centered to the same phone-width lane as Detail/Follow-up so the back button no longer drifts when moving between picks/results/refine on wider screens.
