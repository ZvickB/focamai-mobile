...
- Shopping Region is now intentionally a quick selector, not a long settings/catalog surface. Keep the compact divider-list treatment, with United States/Canada/United Kingdom grouped as `Common regions`, the rest under `More regions`, and a clear selected check state while unselected rows stay secondary and easy to scan. The saved Amazon marketplace behavior and return-to-Search navigation are unchanged.
