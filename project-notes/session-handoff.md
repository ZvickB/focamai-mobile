...
- The Results retry correction surface is an expandable `Improve these picks` prompt. Keep one correction note and one `Update my picks` action. The generated improved query is shown after auto-start on RetryUpdating for transparency, but do not restore an editable generated-query confirmation or correction chips without revisiting the product decision.
