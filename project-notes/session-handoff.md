...
- A temporary dev-only background palette tester now exists for comparing the app canvas across Search, Refine, Results/Picks, and Detail. `src/theme/BackgroundPaletteContext.jsx` owns the remaining warm-neutral options and semantic tokens (`appBackground`, `cardBackground`, `borderSubtle`), `App.js` installs the provider, `RootNavigator.jsx` applies the selected `appBackground` to navigation/header/content colors and renders the `__DEV__` cycle button, and `ScreenContainer` uses the same token for the shared screen shell. Soft Porcelain is currently the default active option after removing Warm Stone, Subtle Oat, and Pale Sand. Search has a temporary Search-only top warmth wash via `backgroundElement`, and Search/Refine now share `SearchFlowProgressCue` for the top `Search / Refine / Picks` step cue. This is deliberately in-memory only, not a persisted user setting, and should be removed once the background choice is made.
