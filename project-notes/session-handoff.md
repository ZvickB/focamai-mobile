...
- Results/Picks no longer use an expanded top-pick card. `src/search/SearchResultRows.jsx` now renders all focused picks as equal ranked rows/cards with the same image lane, title/reason limits, metadata line, and `View details` affordance. `src/screens/ResultsScreen.jsx` owns a sticky selected-result hero panel above the scrolling rows; selection follows the highest visible result row while scrolling and updates the selected title/image/metadata. The selected hero panel is side-by-side, keeps the product image large and slightly lifted, makes title plus rationale the main focus, and pushes price/rating lower as quieter metadata. The smaller rows are lighter scan cards with reduced height, quieter rationale copy, compressed metadata, and softened image framing. Long selected titles currently get a larger selected-panel allowance and are clamped at a word boundary before rendering so they end with a cleaner `...` rather than cutting a word in half where practical. The selected row is marked with a left accent line, lighter cream shading, and an accessibility label. This is an intentional mobile direction change away from the May 19 featured-first layout; backend contracts, controller flow, detail navigation, retailer disclosure, retry behavior, and the 6-result cap are unchanged.
...
- Focused result rows have a restrained hierarchy tune: smaller two-line product titles, more readable AI rationale teasers above quieter price/rating/review/provider metadata, slightly more row padding/text-image spacing, and a bit more image inset inside the existing compact frame. The compact list structure, surfaces, colors, detail navigation, retailer disclosure behavior, and 6-result cap are unchanged.
