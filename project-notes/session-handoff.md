...
- Android Play-readiness audit completed on 2026-05-26. The code-health signals are good, but this is not a final Play-ready release candidate yet. Before Google Play internal testing/final submission, add the Android package id and `versionCode`, create EAS build config, wire production `EXPO_PUBLIC_API_BASE_URL` for EAS builds, complete Play Console app-content/Data safety/store-listing work, and verify a signed installed build. Details live in `project-notes/android-play-readiness-audit-2026-05-26.md`.
