...
- Finalized mobile product detail now owns the same two optional entry points as web. Price Watch requires an ASIN plus positive current price and writes directly to the RLS-protected `price_watches` table; Settings -> Price watches manages the account's five watches. Alerts are email-only, paused watches still count, and the mobile `Alert baseline` label intentionally reflects the backend's post-notification baseline reset.
